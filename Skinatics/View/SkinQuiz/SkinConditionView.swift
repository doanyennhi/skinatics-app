...
struct SkinConditionView: View {
    ...
    var body: some View {
        QuizTemplate(title: "Any skin conditions?", subheading: "(Optional) Select any conditions you may have", show: false, options: conditions, selected: $selected, screen: HomeView().navigationBarBackButtonHidden(), btnText: "Submit", selectionOptional: true, action: {user.skinConditions = selected})
    }
}
